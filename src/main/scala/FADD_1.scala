...
class MY_ADD extends Module{
    ...
    mulAddRecFNToRaw_preMul.io.b  := UInt(BigInt(1)<<31)
    mulAddRecFNToRaw_preMul.io.c  := recFNFromFN(8, 24, io.b)
...
class TesterSimple_5 (dut:MY_ADD) extends PeekPokeTester(dut){
...
object fadd_1 extends App {
  chisel3.iotesters.Driver.execute(args, () => new MY_ADD())(c => new TesterSimple_5(c))
}
